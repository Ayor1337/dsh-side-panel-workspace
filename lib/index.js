 ...
 *  3. 提供 /drawer/vendor/* 静态路由，向浏览器半分发 @xterm/xterm 的 ESM/CSS 与
 *     @highlightjs/cdn-assets 的 UMD 单文件（语法高亮，无构建步骤）；
 *  4. 提供 /drawer/fs/* 只读路由（list/read/search），供资源管理器浏览/搜索工作区文件
 *     （1MB 读上限 + 二进制检测；搜索全工作区递归、200 条命中即停）。
 *
...
const MAX_SEARCH = 200; // 文件名搜索命中上限（全工作区递归，命中即停防拖垮宿主）
...
async function loadVendor(name) {
	...
	const table = {
		...
		"highlight.min.js": ["../node_modules/@highlightjs/cdn-assets/highlight.min.js", "text/javascript; charset=utf-8"]
	};
...
export function apply(ctx) {
	...
	// GET /drawer/vendor/* —— 向浏览器半分发 @xterm/xterm 与 @highlightjs/cdn-assets（无构建步骤）
	...
	route("/drawer/vendor/highlight.min.js", vendorRoute("highlight.min.js"));
	...
	// GET /drawer/fs/search?root=&q= —— 递归全工作区按文件名子串搜索（大小写不敏感，不排除
	// node_modules/隐藏目录；符号链接目录跳过防环；命中 MAX_SEARCH 即停并置 truncated）
	route("/drawer/fs/search", guard(async (req, res) => {
		if (req.method !== "GET") {
			respond(res, 405, { ok: false, error: "method not allowed" });
			return;
		}
		...
		if (!isAbsolute(root)) {
			respond(res, 400, { ok: false, error: "root 必须是绝对路径" });
			return;
		}
		if (query.length < 1 || query.length > 100) {
			respond(res, 400, { ok: false, error: "q 长度须为 1-100 字符" });
			return;
		}
		let info = null;
		try {
		...
		if (info === null || !info.isDirectory()) {
			respond(res, 404, { ok: false, error: "目录不存在：" + root });
			return;
		}
		const needle = query.toLowerCase();
		const entries = [];
		let truncated = false;
		const queue = [root]; // 广度优先：先浅后深，结果更贴近用户目录
		outer: while (queue.length > 0) {
			...
			try {
				...
				continue; // 无权限/已删除目录：跳过该子树继续搜索
			}
			for (const entry of dirents) {
				if (entries.length >= MAX_SEARCH) {
					truncated = true;
					break outer;
				}
				const child = join(dirPath, entry.name);
				...
				if (dir && !entry.isSymbolicLink()) queue.push(child); // 符号链接目录不递归（防环）
			}
		}
		respond(res, 200, { ok: true, root, q: query, truncated, entries });
	}));
