...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* 惰性加载 highlight.js（vendor 路由分发的 UMD 单文件；浏览器原生 ESM 下走全局挂载）；
		   加载失败（未 pnpm install / 404）返回 null，FileView 降级纯文本 */
		let hljsPromise = null;
		const loadHljs = () => {
			if (hljsPromise === null) {
				hljsPromise = import("/drawer/vendor/highlight.min.js")
					.then(() => (typeof globalThis !== "undefined" && globalThis.hljs !== undefined ? globalThis.hljs : null))
					.catch(() => null);
			}
			return hljsPromise;
		};
