 ...
 *    支持多开（标签名跟随当前域名）；不记忆网址（每次新建从空白开始）。
 *  - 资源管理器：左侧文件内容显示区（highlight.js 语法高亮，vendor 路由分发 UMD 单文件，
 *    加载失败降级纯文本）+ 右侧工作区文件树（根=当前会话 cwd，懒加载，文件夹左键展开/折叠，
 *    扩展名类别图标，顶部文件名搜索=宿主 /drawer/fs/search）；点击文件开文件标签（单例）；
 *    文件标签带工具栏（面包屑纯显示 + 树折叠按钮），树宽可拖动（160–480）。
 * 全部取 DSH 主题变量（--dsw-*）。
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			/* ── 资源管理器（左内容 + 右树） ── */
			".sdw_explorerWrap{flex:1;min-height:0;display:flex;flex-direction:column}",
			...
			".sdw_fileBarMeta{flex:none;margin-left:auto}",
			".sdw_pre{flex:1;min-height:0;margin:0;padding:8px 12px;overflow-x:hidden;overflow-y:auto;font:12px/1.6 ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono','Courier New',monospace;color:var(--dsw-alias-label-primary,#1f2328);white-space:pre-wrap;overflow-wrap:anywhere}",
			".sdw_explorerRight{flex:none;min-width:0;display:flex;flex-direction:column;border-left:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.1))}",
			...
			".sdw_treeCaretFold{transform:rotate(180deg)}",
			".sdw_treeName{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0}",
			".sdw_treeWrap{flex:1;min-height:0;display:flex;flex-direction:column}",
			...
			/* ── 工具栏（文件标签页：面包屑 + 折叠按钮） ── */
			".sdw_toolbar{display:flex;align-items:center;gap:6px;padding:5px 10px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.1));font-size:12px;flex:none;min-width:0}",
			".sdw_crumbs{display:flex;align-items:center;gap:2px;min-width:0;flex:1;overflow:hidden;white-space:nowrap;color:var(--dsw-alias-label-secondary,#4b5563)}",
			".sdw_crumbItem{display:inline-flex;align-items:center;gap:2px;min-width:0;flex-shrink:1}",
			".sdw_crumbItem:last-child{flex:none}",
			".sdw_crumb{flex:none;overflow:hidden;text-overflow:ellipsis}",
			".sdw_crumbLast{color:var(--dsw-alias-label-primary,#1f2328);font-weight:600;flex-shrink:1}",
			".sdw_crumbSep{flex:none;color:var(--dsw-alias-label-tertiary,#8a9199)}",
			/* ── 搜索栏与结果列表 ── */
			".sdw_treeSearch{padding:6px 8px 4px;flex:none}",
			".sdw_treeSearch input{width:100%;box-sizing:border-box;height:26px;padding:0 10px;border:0;border-radius:6px;background:var(--dsw-alias-bg-layer-2,#f0f0f0);color:var(--dsw-alias-label-primary,#1f2328);font:inherit;font-size:12px;outline:none}",
			".sdw_treeSearch input:focus{box-shadow:inset 0 0 0 1px var(--dsw-alias-state-business-primary,#3b82f6)}",
			".sdw_treeSearch input::placeholder{color:var(--dsw-alias-label-tertiary,#8a9199)}",
			".sdw_searchParent{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary,#8a9199);font-size:11px;text-align:right}",
			/* ── 树宽拖动分隔条 ── */
			".sdw_treeResizer{position:relative;width:4px;flex:none;cursor:col-resize;background:transparent}",
			".sdw_treeResizer:hover,.sdw_treeResizerOn{background:var(--dsw-alias-state-business-primary,#3b82f6);opacity:.5}",
			".sdw_treeResizer:before{content:'';position:absolute;left:-2px;right:-2px;top:0;bottom:0}",
			/* ── 语法高亮主题（自写 token 配色，取 DSH 变量，明暗自动适配） ── */
			".sdw_pre .hljs-comment,.sdw_pre .hljs-quote{color:var(--dsw-alias-label-tertiary,#8a9199);font-style:italic}",
			".sdw_pre .hljs-keyword,.sdw_pre .hljs-selector-tag,.sdw_pre .hljs-tag{color:var(--dsw-alias-state-business-primary,#3b82f6)}",
			".sdw_pre .hljs-string,.sdw_pre .hljs-regexp,.sdw_pre .hljs-addition{color:var(--dsw-alias-state-success-primary,#16a34a)}",
			".sdw_pre .hljs-number,.sdw_pre .hljs-literal,.sdw_pre .hljs-symbol{color:var(--dsw-alias-state-warning-primary,#d97706)}",
			".sdw_pre .hljs-title,.sdw_pre .hljs-section{color:var(--dsw-alias-state-purple-primary,#7c3aed)}",
			".sdw_pre .hljs-attr,.sdw_pre .hljs-attribute,.sdw_pre .hljs-variable,.sdw_pre .hljs-template-variable{color:var(--dsw-alias-state-error-primary,#e5484d)}",
			".sdw_pre .hljs-built_in,.sdw_pre .hljs-type,.sdw_pre .hljs-class .hljs-title{color:var(--dsw-alias-state-info-primary,#0891b2)}",
			".sdw_pre .hljs-meta,.sdw_pre .hljs-doctag,.sdw_pre .hljs-name{color:var(--dsw-alias-label-secondary,#4b5563)}",
			".sdw_pre .hljs-emphasis{font-style:italic}",
			".sdw_pre .hljs-strong{font-weight:600}"
		].join("");
		...
		/* ── 文件类别图标（按扩展名，固定类别色明暗均可读） ─────────── */
		const FILE_ICON_GROUPS = [
			[["js", "jsx", "ts", "tsx", "mjs", "cjs"], "#f7df1e", "JS"],
			[["json"], "#f5a623", "{}"],
			[["css", "scss", "less"], "#2965f1", "#"],
			[["html", "htm", "xml", "svg"], "#e44d26", "<>"],
			[["md", "markdown"], "#42a5f5", "M"],
			[["py"], "#3776ab", "Py"],
			[["sh", "bash", "zsh", "ps1", "bat", "cmd"], "#4eaa25", ">_"],
			[["png", "jpg", "jpeg", "gif", "ico", "webp", "bmp", "mp3", "mp4", "mov", "avi", "webm"], "#9c27b0", "IMG"],
			[["ini", "toml", "yaml", "yml", "env", "conf", "config", "lock"], "#757575", "*"]
		];
		const kindIcon = (bg, label) => e("svg", { width: 14, height: 14, viewBox: "0 0 16 16", "aria-hidden": true },
			e("rect", { x: 1, y: 1, width: 14, height: 14, rx: 3, fill: bg }),
			e("text", {
				x: 8,
				y: 12,
				textAnchor: "middle",
		...
		/* 按文件名取类别图标：未知类型回退通用文件图标 */
		const iconFileKind = (name) => {
			const dot = name.lastIndexOf(".");
			if (dot <= 0 || dot === name.length - 1) return iconFile();
			const ext = name.slice(dot + 1).toLowerCase();
			for (const [exts, bg, label] of FILE_ICON_GROUPS) {
				if (exts.includes(ext)) return kindIcon(bg, label);
			}
			return iconFile();
		};
		...
		/* ── 浏览器标签页（地址栈导航 + 多开 + iframe 加固；不记忆网址） ── */
		function BrowserPane({ onUrlChange }) {
			...
			/* 不记忆网址：每次新建标签从空白开始页出发 */

			const show = (target) => {
				setFrame(target);
				setUrl(target);
				if (onUrlChange !== undefined) onUrlChange(target);
			};

			/* 地址栏导航：截掉指针之后的前进分支再压栈 */
		...
		/* ── 语法高亮：扩展名 → highlight.js common 语言 id（未列出的类型不调用高亮） ── */
		const HLJS_LANG = {
			...
			ini: "ini", toml: "ini", sql: "sql", java: "java", go: "go", rs: "rust",
			c: "c", h: "c", cpp: "cpp", cc: "cpp", cxx: "cpp", hpp: "cpp",
			kt: "kotlin", lua: "lua", rb: "ruby", php: "php"
		};
		const fileLang = (path) => {
			const dot = path.lastIndexOf(".");
			if (dot <= 0 || dot === path.length - 1) return null;
			return HLJS_LANG[path.slice(dot + 1).toLowerCase()] ?? null;
		};
		/* 惰性加载 highlight.js（vendor 分发的 min 构建为“顶层 var hljs + 仅 CJS 导出”，
		   ESM import() 下模块作用域的 var 不外泄——必须用经典 <script> 注入让 var 挂到 window；
		   失败（未 pnpm install / 404）resolve null，FileView 降级纯文本 */
		let hljsPromise = null;
		const loadHljs = () => {
			if (hljsPromise === null) {
				hljsPromise = new Promise((resolve) => {
					const tag = document.createElement("script");
					tag.src = "/drawer/vendor/highlight.min.js";
					tag.dataset.plugin = "dsh-plugin-side-drawer";
					tag.onload = () => resolve(typeof window !== "undefined" && window.hljs !== undefined ? window.hljs : null);
		...
		function FileTree({ cwd, onOpenFile }) {
			...
			const [query, setQuery] = useState("");       // 搜索输入
			const [results, setResults] = useState(null); // null=未搜索 | { entries, truncated, error }
			const [searching, setSearching] = useState(false);
			const searchSeqRef = useRef(0); // 搜索请求序号：只采纳最新一轮响应
			...
			/* 搜索防抖 300ms：空输入清空结果恢复树；晚到的旧响应按序号丢弃 */
			useEffect(() => {
				const q = query.trim();
				if (q === "") {
				...
				const timer = setTimeout(async () => {
					setSearching(true);
					try {
						const data = await fetchJson("/drawer/fs/search?root=" + encodeURIComponent(cwd) + "&q=" + encodeURIComponent(q));
						if (searchSeqRef.current === seq) setResults({ entries: data.entries ?? [], truncated: data.truncated === true, error: null });
					} catch (err) {
						if (searchSeqRef.current === seq) setResults({ entries: [], truncated: false, error: err instanceof Error ? err.message : String(err) });
					} finally {
			...
			const renderEntry = (entry, depth) => {
				...
				return e("button", {
					...
					iconFileKind(entry.name),
					e("span", { className: "sdw_treeName" }, entry.name));
			};

			/* 结果行父目录（相对 cwd，去掉共同前缀与文件名） */
			const parentText = (p) => {
				const parts = p.split(/[\\/]/).filter(Boolean);
				parts.pop();
				if (parts.length === 0) return "";
				const rootParts = (typeof cwd === "string" ? cwd : "").split(/[\\/]/).filter(Boolean);
				while (parts.length > 0 && rootParts.length > 0 && parts[0] === rootParts[0]) {
					parts.shift();
					rootParts.shift();
				}
				return parts.join("/");
			...
			/* 树主体（无搜索时） */
			const renderTree = () => {
				if (typeof cwd !== "string" || cwd === "") return e("div", { className: "sdw_treeNote" }, "当前会话无工作目录");
			...
			/* 搜索结果列表（激活时替代树主体；目录项仅展示不可点击） */
			const renderResults = () => {
				if (results.error !== null) return e("div", { className: "sdw_treeNote sdw_treeErr" }, results.error);
				if (searching && results.entries.length === 0) return e("div", { className: "sdw_treeNote" }, "搜索中…");
				if (results.entries.length === 0) return e("div", { className: "sdw_treeNote" }, "无匹配结果");
				return e("div", { className: "sdw_tree" },
					results.entries.map((entry) => entry.dir
						? e("div", {
							key: entry.path,
							className: "sdw_treeRow",
							...
							iconFolder(),
							e("span", { className: "sdw_treeName" }, entry.name),
							e("span", { className: "sdw_searchParent" }, parentText(entry.path)))
						: e("button", {
							key: entry.path,
							type: "button",
							className: "sdw_treeRow",
							...
							e("span", { className: "sdw_treeName" }, entry.name),
							e("span", { className: "sdw_searchParent" }, parentText(entry.path)))),
					results.truncated ? e("div", { className: "sdw_treeNote" }, "仅显示前 200 条匹配") : null);
			};

			return e("div", { className: "sdw_treeWrap" },
				e("div", { className: "sdw_treeSearch" },
					e("input", {
						value: query,
				...
				results === null
					? (query.trim() !== "" && searching
						? e("div", { className: "sdw_treeNote" }, "搜索中…")
						: renderTree())
					: renderResults());
		}

		/* ── 资源管理器：左侧文件内容视图（语法高亮，hljs 失败降级纯文本） ── */
		function FileView({ path }) {
			const [state, setState] = useState({ phase: "loading", data: null, error: null, html: null });

			useEffect(() => {
				let disposed = false;
				setState({ phase: "loading", data: null, error: null, html: null });
				(async () => {
					try {
						...
						if (data.binary !== true) {
							...
							if (lang !== null) {
								...
								if (hljs !== null) {
									try {
										/* highlight.js 对输入做 HTML 转义后输出 token 化 HTML，dangerouslySetInnerHTML 安全 */
										html = hljs.highlight(data.content ?? "", { language: lang, ignoreIllegals: true }).value;
									} catch {
										html = null; // 语言不可用等 → 纯文本
									}
								}
							}
						}
						if (!disposed) setState({ phase: "done", data, error: null, html });
					} catch (err) {
						if (!disposed) setState({ phase: "error", data: null, error: err instanceof Error ? err.message : String(err), html: null });
					}
				})();
				...
				state.phase === "loading" ? e("div", { className: "sdw_start" }, "正在读取…")
					: state.phase === "error" ? e("div", { className: "sdw_err" }, state.error)
						: state.data.binary === true ? e("div", { className: "sdw_start" },
							...
							: e("pre", { className: "sdw_pre" },
								state.html !== null
									? e("code", { dangerouslySetInnerHTML: { __html: state.html } })
									: (state.data.content ?? "")));
		}

		/* ── 资源管理器标签页（左内容 + 右树；explorer=空态，file=文件内容） ──
		   file 模式在顶部渲染工具栏（面包屑纯显示 + 折叠按钮）；树宽可拖动（160–480） */
		...
		function ExplorerPane({ kind, file, cwd, onOpenFile }) {
			...
			/* 组件卸载时若拖动仍进行，移除 document 级监听并恢复文本选择 */
			useEffect(() => () => {
				if (dragCleanupRef.current !== null) dragCleanupRef.current();
			}, []);

			/* 拖动分隔条：分隔条位于树左边界，向右拖 = 树左边界右移 = 树变窄（宽度 = 起点 - 位移） */
			const onResizerDown = (ev) => {
				ev.preventDefault();
				const startX = ev.clientX;
				const startWidth = treeWidth;
				document.body.style.userSelect = "none";
				const onMove = (me) => setTreeWidth(clampNum(startWidth - (me.clientX - startX), TREE_MIN_WIDTH, TREE_MAX_WIDTH));
				const cleanup = () => {
			...
			/* 面包屑分段（纯显示：路径各段 + 分隔符，末段加粗） */
			const crumbs = kind === "file" && file !== null
				? file.path.split(/[\\/]/).filter(Boolean)
				: [];

			return e("div", { className: "sdw_explorerWrap" },
				kind === "file" && file !== null ? e("div", { className: "sdw_toolbar" },
					e("div", { className: "sdw_crumbs", title: file.path },
						crumbs.map((seg, i) => e("span", { key: i, className: "sdw_crumbItem" },
							i > 0 ? e("span", { className: "sdw_crumbSep" }, "›") : null,
							e("span", { className: "sdw_crumb" + (i === crumbs.length - 1 ? " sdw_crumbLast" : "") }, seg)))),
					e("button", {
						type: "button",
						className: "sdw_btn",
						title: treeVisible ? "折叠树状图" : "展开树状图",
						"aria-label": treeVisible ? "折叠树状图" : "展开树状图",
						"aria-expanded": treeVisible,
						onClick: () => setTreeVisible(!treeVisible)
					},
						e("span", { className: "sdw_treeCaret" + (treeVisible ? " sdw_treeCaretOpen" : " sdw_treeCaretFold") }, iconCaret())))
					: null,
				e("div", { className: "sdw_explorer" },
					...
					treeVisible ? e("div", { className: "sdw_treeResizer", onMouseDown: onResizerDown, role: "separator", "aria-orientation": "vertical" }) : null,
					treeVisible ? e("div", { className: "sdw_explorerRight", style: { width: treeWidth } },
						e(FileTree, { cwd, onOpenFile })) : null));
		}
		...
		function DrawerPanel({ subscribe, actions, getCwd, subscribeCwd }) {
			...
			/* 初始无标签：展开抽屉默认什么都不启动，由用户在首页手动新建终端/浏览器/资源管理器 */
			...
			/* 标签标题/图标：terminal 取 cwd 末级名；browser 取当前域名（未导航显示"新标签"）；
			   explorer 固定名；file 取文件名 */
			const urlHost = (raw) => {
				try { return new URL(raw).host; } catch { return raw; }
			};
			const tabTitle = (tab) => tab.kind === "terminal" ? cwdName()
				: tab.kind === "browser" ? (tab.url !== undefined && tab.url !== "" ? urlHost(tab.url) : "新标签")
			...
			/* 浏览器多开：各标签独立上报当前网址，驱动标签名（按 key 更新，避免无谓重渲染） */
			const updateTabUrl = (key, url) => {
				setTabs((prev) => {
			...
			const addTab = (kind) => {
				...
				if (kind === "explorer") {
					...
					if (existing !== undefined) { setActiveKey(existing.key); return; } // 资源管理器单例：已存在只切换
				}
					...
					tabs.map((tab) =>
						e("div", { key: tab.key, className: "sdw_pane", style: { display: activeKey === tab.key ? "flex" : "none" } },
							tab.kind === "terminal"
								...
								: tab.kind === "browser"
									? e(BrowserPane, {
										visible: open && activeKey === tab.key,
										onUrlChange: (url) => updateTabUrl(tab.key, url)
									})
									: e(ExplorerPane, {
										kind: tab.kind,
